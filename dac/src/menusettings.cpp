...
static void Voltage_Select(void)
{
    ...
    uint8_t err = 0;
    err = EEPROM_read_batch(VOLTAGE_ADDR, &voltage,1);
...
{
    viewTimeDate(SHOW_NONE);
    EEPROM_read_batch(CURRENT_ADDR, &current, 1);
...
static void Current_Save() {
    LCDclr();
    EEPROM_update_batch(CURRENT_ADDR, &current,1);
...
static void Voltage_Save() {
    LCDclr();
    EEPROM_update_batch(VOLTAGE_ADDR, &voltage,1);
